...
const CategoryDetail = ({ item }: CategoryDetailProps) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (res[0].code === 200) {
        // Topic Overview response
        setTopicOverview(res[0]?.data?.topic_overview ?? null);
      }
      if (res[1].code === 200) {
        // Recommended Contents response
  ...
  useEffect(() => {
    ...
    const nShowComments = nFeedbacks.filter((feedback) => feedback.showComments);
    if (nShowComments.length) {
      nShowComments.forEach((nShowComment) => {
        let comments = document.querySelector(`#feedback-${nShowComment.id}-comments`) as HTMLDivElement;
        valueMemberAccordion.style.maxHeight = valueMemberAccordion.clientHeight +
          comments.clientHeight + "px";
      });
    }
  }, [nFeedbacks]);
  ...
  const filterVendorsByCategory = (cids: number[]) => {
    ...
    searchVendorsAPI(token ?? "", { q: "", filters: cids })
      .then((res) => {
        setLoading(!1);
        if (res.code === 200) {
                    ...
                    <div className="flex justify-between flex-wrap">
                      <div className="mt-2 mb-4 w-full lg:w-6/12">
                        ...
                        <div className="right my-4">
                          <button
                            className="bg-theme active:bg-blueGray-600 text-white font-bold uppercase text-xs px-4 py-3 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150"
                            type="button"
